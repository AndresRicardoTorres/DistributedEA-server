...
function Server(database,mainCallback){
  ...
  function initPopulation(callback){
	...
	population[aDocument.generation]=documents[i];		
      }
	...
	if(countIndividualsInGeneration(possibleGeneration) < project.populationTotal){	  
  ...
  function createInitialPopulation(){
    ...
    var count = countIndividualsInGeneration(0);
  ...
  function initialize(){
	...
	initPopulation(function(populationSuccess){
	  ...
	  var ammountInGeneration0 = countIndividualsInGeneration(0);
  ...
  function countInGeneration(generationNumber){
    return populationCountByGeneration[generationNumber];
  }
