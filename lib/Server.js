...
function Server(database, projectName, mainCallback) {
  ...
  function initProject(callback) {
    ...
    projectsCollection.findOne(filter, function (err, doc) {
      ...
      if (doc !== null) {
        ...
        project.stopFitnessZero    = doc.stopFitnessZero;
  ...
  function handleRequest(callback) {
    if (actualGeneration < project.generationLimit && !project._zero) {
      var realAmmount = 0,
...
//    last = j;
  ...
  this.mutateSharedFunctions = function (callback) {
    functionsCollection.find({}).toArray(function (err, functions) {
      var i = 0, random = 0, toMutate = null, key = null;
      if (err) {
        console.error(err);
      }
      for (i = 0; i < functions.length; i += 1) {

        if (functions[i].counter[projectName] === undefined) {
          functions[i].counter[projectName] = 0;
        }

        functions[i].used[projectName] = false;
        if (functions[i].use) {
          functions[i].counter[projectName] += 1;
          functions[i].use = false;
          functions[i].used[projectName] = true;
        } else {
          toMutate = true;
          for (key in functions[i].used) {
            if (functions[i].used.hasOwnProperty(key)) {
              if (functions[i].used[key]) {
                toMutate = false;
              }
            }
          }
          if (toMutate) {
            random = Math.random();
            if (random < project.mutationSharedPercent) {
  ...
  function handleDeliver(request, callback) {
    ...
    if (countInGeneration(actualGeneration) ===
        countInGeneration(actualGeneration + 1)) {
      actualGeneration += 1;
      callback({changeGeneration: true});
    } else {
      callback({changeGeneration: false});
    }
  }
  ...
  function searchBestIndividuals(beginGeneration, endGeneration) {
    var manyZero    = 0,
      i             = 0,
      generation    = -1,
      countFunction = function (aIndividual) {
    ...
    for (i = beginGeneration; i < endGeneration; i++) {
      if (population[i] !== 'undefined') {
        population[i].forEach(countFunction);
        if (manyZero > 0) {
          console.log("");
  ...
  this.writeInDB = function () {
    if (!lockWriteInDB) {
      ...
      bestGeneration = searchBestIndividuals(actualGenerationInDB, limit);
      if (bestGeneration !== -1) {
        /// Found a best, then only write until the bestGeneration
        limit = bestGeneration;
        for (i = actualGenerationInDB + 1; i < limit; i += 1) {
          insert_population = insert_population.concat(population[i]);
        }
        // Send stop the project
        if (project.stopFitnessZero) {
          project._zero = true;
        }
      }
  ...
  this.processCommunication = function (request, callback) {
    ...
    if (request.action !== undefined) {
      ...
      case 'deliver':
        ...
        handleDeliver(request, function (state) {
          response.changeGeneration = state.changeGeneration;
          response.ok = true;
          callback(null, response);
        });
        break;
