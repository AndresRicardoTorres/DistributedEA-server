function Server(database, mainCallback) {
  ...
  function initPopulation() {
    ...
    var i = 0;
    for (i = 0; i <= project.generationLimit; i++) {
      population[i] = [];
    }
  }
  ...
  function createInitialPopulation() {
    ...
    var i           = 0;
    var aChromosome = null;
    var aIndividual = null;

    for (i = count; i < project.populationTotal; i++) {
  ...
  var requestRandomInteger = function (max) {
  ...
  function getPossiblePositions() {
    ...
    for (i = 0; i < population[actualGeneration].length; i++) {
  ...
  function handleRequest(callback) {
    if (actualGeneration < project.generationLimit) {
      var difference = project.populationTotal
  ...
  function requestToIndividuals(request) {
    ...
    var limit                        = Math.min(project.populationTotal,
      request.newChromosomes.length + countIndividualsInGeneration);
    var i                            = 0;
    var aIndividual                  = null;

    for (i = countIndividualsInGeneration; i < limit; i++) {
      aIndividual = {
        _id        : ++sequenceId,
...
//     var sortNumbers = function(a,b){if(a<b)return -1;if(a>b)return 1;return 0};
    ...
    for (i = 0; i < ids.length; i++) {
      for (j = last; j < populationInGeneration.length; j++) {
...
/*
  ...
  this.writeInDB = function () {
    if (!lockWriteInDB) {
      ...
      for (i = actualGenerationInDB + 1; i < limit; i++) {
  ...
  this.totalTimeRequest = 0;
  this.totalTimeDeliver = 0;
  this.processCommunication = function (request, callback) {
    ...
    if (request.action !== undefined) {
      ...
      case 'request':
        var handleRequestTime = new Date();
        handleRequest(function (project, subPopulation, oldIds) {
          ...
          handleRequestTime      = new Date() - handleRequestTime;
          this.totalTimeRequest += handleRequestTime;
      ...
      case 'deliver':
        ...
        var handleDeliverTime  = new Date();

        handleDeliver(request, function () {
          handleDeliverTime  = new Date() - handleDeliverTime;
          this.totalTimeDeliver  += handleDeliverTime;
