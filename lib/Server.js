...
// 	  console.log(project.populationTotal,'project.populationTotal');
	  ...
	  populationCollection.insert(individuals,{w:"majority"},function(err){
...
// 	    console.log('actualGeneration='+generation);
  ...
  this.handleDeliver= function(request,callback){
    ...
    populationCollection.count({generation:request.generation},function(err,countGeneration){
      console.log("Hay "+countGeneration+" en la generation "+request.generation+" en la BD"+{generation:request.generation});
      console.log(request.fitness,'fitness');
      
      var individuals = new Array();
      
      for(var i=countGeneration;i<request.newChromosomes.length;i++){
	
	  var aIndividual={
	    generation:parseInt(request.generation),
	    position:countGeneration+i+1,
	    chromosome:request.newChromosomes[i],
	    fitness:parseInt(request.fitness[i])
	  };
	  individuals.push(aIndividual);
      }
      if(individuals.length > 0){
	console.log("INTENTO insertar "+individuals.length);
	populationCollection.insert(individuals,{w:"majority"},function(err,docs){
